...
{
    internal sealed class EventStoreRepository<T> : IEventStoreRepository<T>
        where T : IEventBaseAggregate<Guid>, new()
    {
            ...
            Task<T> ExecuteAsync() =>
                Task.Run(
                    () =>
                    {
                        ...
                        using var eventStore = this.context.Events;
                        var snapshot = eventStore.Advanced.GetSnapshot(id, int.MaxValue);
                        if (snapshot == null)
                        {
                            using var stream = eventStore.OpenStream(id, 0);
                            var events = from s in stream.CommittedEvents select s.Body as ISyncDomainEvent;
            ...
            Task ExecuteAsync() =>
                Task.Run(
                    () =>
                    {
                        ...
                        using var eventStore = this.context.Events;
                        using var stream = eventStore.OpenStream(root.Id, 0);
        ...
        public Task SaveAsync(IEventBaseAggregate<Guid> root)
        {
            ...
            return this.SaveAsync(root, guid);
        }
    }
}